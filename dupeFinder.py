...
from common import clio, PerfTimer
...
def summarize(path, dupeList, diffList, uniqList):
    ...
    contents = {
        'subs':{}, 'dupes':dupeList, 'diffs':diffList, 'uniqs':uniqList, 'dupes_c':dc, 'diffs_c':fc, 'uniqs_c':uc
    }
...
def getPaths(path):
    ...
    if os.path.isdir(path):
        for sub in os.listdir(path):
            paths.extend(getPaths(path + os.path.sep + sub))
...
def getFilteredPaths(path, ext, sort=True, reverse=False, recurseDirs=False):
    paths = getPaths(path) if recurseDirs else os.listdir(path)
    ...
    if reverse:
        filtered = [fname for fname in paths if not fileFilter.match(fname)]
    else:
